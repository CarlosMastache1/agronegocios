...
if (tabla_produccion_producto && strongTagsM.length > 0) {
  strongTagsM[0].textContent = `${summun} municipio`;
}
...
if (strongTagsP.length > 0) {
  strongTagsP[0].textContent = `${pub_indigenas_producto[0].length } pueblo indígena`;
}
...
if (oaxacaData && strongTagsD.length > 1) {
  strongTagsD[1].textContent = `${summun} municipio`;
  strongTagsD[2].textContent = `${oaxacaData.ranking}° lugar`;
  strongTagsD[3].textContent = `${pub_indigenas_producto[0].length  } pueblo indígena`;
}
