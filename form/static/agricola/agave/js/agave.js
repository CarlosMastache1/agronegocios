...
function renderTable() {
  ...
  for (const row of pageData) {
    ...
    tr.innerHTML = `<td>${row.ranking}</td><td>${
      ...
      "es-MX"
    )}</td><td>${row.producto.toLocaleString("es-MX")}</td>`;
...
total_datos_tab2.innerHTML = `<th></th><th>Total:</th><th>2,577,995</th><th>239,271</th><th>53.92</th><th>48,284,822,743</th>`;
...
total_datos_tabDesExp.innerHTML = `<th></th><th>Total:</th><th>119,496,547</th>`;

/* TABLA DESTINO DE LA EXPORTACIÓN 2023 POR REGIÓN */
const tab_cuer_region_exp = document.getElementById("tab_cuer_region_exp");
let sumavaloexp = 0;
for (let i = 0; i < tab_region_exportacion.length; i++) {
  const item = tab_region_exportacion[i];
  sumavaloexp += item.valor;

  const fila = document.createElement("tr");

  const celdaNum = document.createElement("td");
  celdaNum.textContent = item.num;

  const celdaRegion = document.createElement("td");
  celdaRegion.textContent = item.region;
  ...
  tab_cuer_region_exp.appendChild(fila);
}

const tfoot_cuer_region_exp = document.getElementById("tfoot_cuer_region_exp");
tfoot_cuer_region_exp.innerText = sumavaloexp.toLocaleString("es-MX");
...
total_tab_pro_miel_oax.innerHTML = `<th>Estatal</th><th>${volumen_producto.toLocaleString(
  ...
  "es-MX"
)}</th><th>${51.91}</th><th>${valor_producto.toLocaleString(
  "es-MX"
...
/* GRAFICA 3 Estacionalidad de agave en Oaxaca */
