const regionesConMunicipios = {
  //73
  Costa: [
    //26
    {
      id: 20334,
      volumen: 2016,
      nombre: "Villa de Tututepec de Melchor Ocampo",
    },
    {
      id: 20482,
      volumen: 1568,
      nombre: "Santiago Pinotepa Nacional",
    },
    {
      id: 20637,
      volumen: 1529,
      nombre: "San Pedro Mixtepec",
    },
    {
      id: 20467,
      volumen: 1159,
      nombre: "Santiago Jamiltepec",
    },
    {
      id: 20439,
      volumen: 677,
      nombre: "Santa María Tonameca",
    },
    {
      id: 20414,
      volumen: 645,
      nombre: "Santa María Huazolotitlán",
    },
    {
      id: 20401,
      volumen: 640,
      nombre: "Santa María Colotepec",
    },
    {
      id: 20485,
      volumen: 473,
      nombre: "Santiago Tapextla",
    },
    {
      id: 20507,
      volumen: 458,
      nombre: "Santo Domingo Armenta",
    },
    {
      id: 20185,
      volumen: 453,
      nombre: "San Juan Cacahuatepec",
    },
    {
      id: 20090,
      volumen: 449,
      nombre: "San Andrés Huaxpaltepec",
    },
    {
      id: 20345,
      volumen: 449,
      nombre: "San Sebastián Ixcapa",
    },
    {
      id: 20180,
      volumen: 443,
      nombre: "San Juan Bautista Lo de Soto",
    },
    {
      id: 20082,
      volumen: 409,
      nombre: "San Agustín Chayuco",
    },
    {
      id: 20402,
      volumen: 408,
      nombre: "Santa María Cortijo",
    },
    {
      id: 20466,
      volumen: 404,
      nombre: "Santiago Ixtayutla",
    },
    {
      id: 20489,
      volumen: 393,
      nombre: "Santiago Tetepec",
    },
    {
      id: 20285,
      volumen: 371,
      nombre: "San Miguel Tlacamama",
    },
    {
      id: 20474,
      volumen: 367,
      nombre: "Santiago Llano Grande",
    },
    {
      id: 20188,
      volumen: 307,
      nombre: "San Juan Colorado",
    },
    {
      id: 20302,
      volumen: 307,
      nombre: "San Pedro Atoyac",
    },
    {
      id: 20526,
      volumen: 280,
      nombre: "Santos Reyes Nopala",
    },
    {
      id: 20312,
      volumen: 279,
      nombre: "San Pedro Jicayán",
    },
    {
      id: 20056,
      volumen: 263,
      nombre: "Mártires de Tacubaya",
    },
    {
      id: 20070,
      volumen: 247,
      nombre: "Pinotepa de Don Luis",
    },
    {
      id: 20324,
      volumen: 242,
      nombre: "San Pedro Pochutla",
    },
  ],
  Istmo: [
    //19
    {
      id: 20057,
      volumen: 6406,
      nombre: "Matías Romero Avendaño",
    },
    {
      id: 20043,
      volumen: 1541,
      nombre: "Heroica Ciudad de Juchitán de Zaragoza",
    },
    {
      id: 20005,
      volumen: 1347,
      nombre: "Asunción Ixtaltepec",
    },
    {
      id: 20198,
      volumen: 722,
      nombre: "San Juan Guichicovi",
    },
    {
      id: 20525,
      volumen: 570,
      nombre: "Santo Domingo Zanatepec",
    },
    {
      id: 20407,
      volumen: 435,
      nombre: "Santa María Chimalapa",
    },
    {
      id: 20066,
      volumen: 416,
      nombre: "Santiago Niltepec",
    },
    {
      id: 20030,
      volumen: 375,
      nombre: "El Espinal",
    },
    {
      id: 20515,
      volumen: 358,
      nombre: "Santo Domingo Tehuantepec",
    },
    {
      id: 20010,
      volumen: 354,
      nombre: "El Barrio de la Soledad",
    },
    {
      id: 20327,
      volumen: 311,
      nombre: "San Pedro Tapanatepec",
    },
    {
      id: 20557,
      volumen: 299,
      nombre: "Unión Hidalgo",
    },
    {
      id: 20265,
      volumen: 295,
      nombre: "San Miguel Chimalapa",
    },
    {
      id: 20427,
      volumen: 289,
      nombre: "Santa María Petapa",
    },
    {
      id: 20141,
      volumen: 275,
      nombre: "San Francisco del Mar",
    },
    {
      id: 20143,
      volumen: 267,
      nombre: "San Francisco Ixhuatán",
    },
    {
      id: 20124,
      volumen: 238,
      nombre: "San Blas Atempa",
    },
    {
      id: 20505,
      volumen: 234,
      nombre: "Santo Domingo Ingenio",
    },
    {
      id: 20513,
      volumen: 216,
      nombre: "Santo Domingo Petapa",
    },
  ],
  Papaloapan: [
    //15
    {
      id: 20184,
      volumen: 2045,
      nombre: "San Juan Bautista Tuxtepec",
    },
    {
      id: 20044,
      volumen: 1549,
      nombre: "Loma Bonita",
    },
    {
      id: 20498,
      volumen: 1456,
      nombre: "Santiago Yaveo",
    },
    {
      id: 20205,
      volumen: 1109,
      nombre: "San Juan Lalana",
    },
    {
      id: 20002,
      volumen: 1007,
      nombre: "Acatlán de Pérez Figueroa",
    },
    {
      id: 20559,
      volumen: 935,
      nombre: "San Juan Bautista Valle Nacional",
    },
    {
      id: 20468,
      volumen: 766,
      nombre: "Santiago Jocotepec",
    },
    {
      id: 20166,
      volumen: 754,
      nombre: "San José Chiltepec",
    },
    {
      id: 20417,
      volumen: 683,
      nombre: "Santa María Jacatepec",
    },
    {
      id: 20134,
      volumen: 636,
      nombre: "San Felipe Jalapa de Díaz",
    },
    {
      id: 20232,
      volumen: 613,
      nombre: "San Lucas Ojitlán",
    },
    {
      id: 20278,
      volumen: 607,
      nombre: "San Miguel Soyaltepec",
    },
    {
      id: 20009,
      volumen: 467,
      nombre: "Ayotzintepec",
    },
    {
      id: 20309,
      volumen: 360,
      nombre: "San Pedro Ixcatlán",
    },
    {
      id: 20136,
      volumen: 241,
      nombre: "San Felipe Usila",
    },
  ],
  "Sierra Sur": [
    //6
    {
      id: 20277,
      volumen: 641,
      nombre: "Villa Sola de Vega",
    },
    {
      id: 20447,
      volumen: 360,
      nombre: "Santa María Zacatepec",
    },
    {
      id: 20386,
      volumen: 315,
      nombre: "Santa Cruz Zenzontepec",
    },
    {
      id: 20477,
      volumen: 300,
      nombre: "Santiago Minas",
    },
    {
      id: 20073,
      volumen: 257,
      nombre: "Putla Villa de Guerrero",
    },
    {
      id: 20300,
      volumen: 209,
      nombre: "San Pedro Amuzgos",
    },
  ],
  "Sierra De Juárez": [
    //3
    {
      id: 20190,
      volumen: 1800,
      nombre: "San Juan Cotzocón",
    },
    {
      id: 20207,
      volumen: 755,
      nombre: "San Juan Mazatlán",
    },
    {
      id: 20200,
      volumen: 246,
      nombre: "San Juan Juquila Mixes",
    },
  ],
  "Valles Centrales": [
    //2
    {
      id: 20449,
      volumen: 296,
      nombre: "Santa María Zoquitlán",
    },
    {
      id: 20295,
      volumen: 203,
      nombre: "San Pablo Huixtepec",
    },
  ],
  "Sierra De Flores Magón": [
    //1
    {
      id: 20406,
      volumen: 482,
      nombre: "Santa María Chilchotla",
    },
  ],
  Mixteca: [
    //1
    {
      id: 20039,
      volumen: 200,
      nombre: "Heroica Ciudad de Huajuapan de León",
    },
  ],
};

/* LECH DE  BOVINO */
const tab_mex_sub_pro = [
  {
    ranking: 1,
    id: "JAL",
    entidad: "Jalisco",
    volumen: 2799041000,
    precio: 8,
    valor: 23763846000,
  },
  {
    ranking: 2,
    id: "COA",
    entidad: "Coahuila",
    volumen: 1537748000,
    precio: 8,
    valor: 13108912000,
  },
  {
    ranking: 3,
    id: "DGO",
    entidad: "Durango",
    volumen: 1515250000,
    precio: 8,
    valor: 13039810000,
  },
  {
    ranking: 4,
    id: "CHI",
    entidad: "Chihuahua",
    volumen: 1262218000,
    precio: 9,
    valor: 11829683000,
  },
  {
    ranking: 5,
    id: "GTO",
    entidad: "Guanajuato",
    volumen: 912848000,
    precio: 8,
    valor: 7587141000,
  },
  {
    ranking: 6,
    id: "VER",
    entidad: "Veracruz",
    volumen: 807075000,
    precio: 8,
    valor: 6533342000,
  },
  {
    ranking: 7,
    id: "CHS",
    entidad: "Chiapas",
    volumen: 463510000,
    precio: 7,
    valor: 3349511000,
  },
  {
    ranking: 8,
    id: "PUE",
    entidad: "Puebla",
    volumen: 462639000,
    precio: 8,
    valor: 3601248000,
  },
  {
    ranking: 9,
    id: "AGS",
    entidad: "Aguascalientes",
    volumen: 440729000,
    precio: 8,
    valor: 3568553000,
  },
  {
    ranking: 10,
    id: "EM",
    entidad: "México",
    volumen: 434103000,
    precio: 9,
    valor: 3593948000,
  },
  {
    ranking: 11,
    id: "HGO",
    entidad: "Hidalgo",
    volumen: 429857000,
    precio: 8,
    valor: 3328284000,
  },
  {
    ranking: 12,
    id: "QRO",
    entidad: "Querétaro",
    volumen: 419822000,
    precio: 9,
    valor: 3630123000,
  },
  {
    ranking: 13,
    id: "MICH",
    entidad: "Michoacán",
    volumen: 382988000,
    precio: 9,
    valor: 3516294000,
  },
  {
    ranking: 14,
    id: "BCN",
    entidad: "Baja California",
    volumen: 212042000,
    precio: 9,
    valor: 1875074000,
  },
  {
    ranking: 15,
    id: "ZAC",
    entidad: "Zacatecas",
    volumen: 194942000,
    precio: 9,
    valor: 1803460000,
  },
  {
    ranking: 16,
    id: "SLP",
    entidad: "San Luis Potosí",
    volumen: 157371000,
    precio: 8,
    valor: 1261388000,
  },
  {
    ranking: 17,
    id: "OAX",
    entidad: "Oaxaca",
    volumen: 151659000,
    precio: 8,
    valor: 1161156000,
  },
  {
    ranking: 18,
    id: "SON",
    entidad: "Sonora",
    volumen: 119593000,
    precio: 9,
    valor: 1048714000,
  },
  {
    ranking: "",
    entidad: "Otros",
    volumen: 629304000,
    precio: 9,
    valor: 5219620000,
  },
];

/* PRODUCCION NACIONAL DEL SECTOR BOVINO */
/* CARNE DE BOVINO */
const tab_mex_pro = [
  {
    ranking: 1,
    id: "VER",
    entidad: "Veracruz",
    volumen: 287065,
    peso: 234,
    precio: 70,
    valor: 20615308000,
  },
  {
    ranking: 2,
    id: "JAL",
    entidad: "Jalisco",
    volumen: 262234,
    peso: 268,
    precio: 85,
    valor: 22582792000,
  },
  {
    ranking: 3,
    id: "SLP",
    entidad: "San Luis Potosí",
    volumen: 134585,
    peso: 251,
    precio: 78,
    valor: 10519874000,
  },
  {
    ranking: 4,
    id: "DGO",
    entidad: "Durango",
    volumen: 121168,
    peso: 240,
    precio: 79,
    valor: 8368448000,
  },
  {
    ranking: 5,
    id: "BCN",
    entidad: "Baja California",
    volumen: 114150,
    peso: 273,
    precio: 69,
    valor: 8207175000,
  },
  {
    ranking: 6,
    id: "CHS",
    entidad: "Chiapas",
    volumen: 113355,
    peso: 230,
    precio: 67,
    valor: 7358110000,
  },
  {
    ranking: 7,
    id: "SIN",
    entidad: "Sinaloa",
    volumen: 112701,
    peso: 246,
    precio: 71,
    valor: 8588649000,
  },
  {
    ranking: 8,
    id: "MICH",
    entidad: "Michoacán",
    volumen: 103858,
    peso: 231,
    precio: 72,
    valor: 7528839000,
  },
  {
    ranking: 9,
    id: "CHI",
    entidad: "Chihuahua",
    volumen: 92718,
    peso: 226,
    precio: 79,
    valor: 8118440000,
  },
  {
    ranking: 10,
    id: "SON",
    entidad: "Sonora",
    volumen: 78646,
    peso: 230,
    precio: 89,
    valor: 7025565000,
  },
  {
    ranking: 11,
    id: "TAB",
    entidad: "Tabasco",
    volumen: 78103,
    peso: 240,
    precio: 82,
    valor: 6410962000,
  },
  {
    ranking: 12,
    id: "NL",
    entidad: "Nuevo León",
    volumen: 77234,
    peso: 268,
    precio: 86,
    valor: 6867241000,
  },
  {
    ranking: 13,
    id: "OAX",
    entidad: "Oaxaca",
    volumen: 67156,
    peso: 195,
    precio: 64,
    valor: 4887111000,
  },
  {
    ranking: 14,
    id: "GTO",
    entidad: "Guanajuato",
    volumen: 59860,
    peso: 251,
    precio: 79,
    valor: 4878040000,
  },
  {
    ranking: "",
    entidad: "Otros",
    volumen: 512095,
    peso: 232,
    precio: 76,
    valor: 39550039000,
  },
];

/* Datos para la segunda tabla sobre la producción de leche de vaca a nivel mundial */
const tabla_paises_sub_prod = [
  //18
  { id: "IND", ranking: 1, pais: "India", volumen: 108371300 },
  {
    id: "USA",
    ranking: 2,
    pais: "Estados Unidos de América",
    volumen: 102721557,
  },
  { id: "CHN", ranking: 3, pais: "China", volumen: 36076655 },
  { id: "BRA", ranking: 4, pais: "Brasil", volumen: 35647495 },
  { id: "CHN", ranking: 5, pais: "China, Continental", volumen: 35613500 },
  { id: "RUS", ranking: 6, pais: "Federación de Rusia", volumen: 32738522 },
  { id: "DEU", ranking: 7, pais: "Alemania", volumen: 32399050 },
  { id: "FRA", ranking: 8, pais: "Francia", volumen: 23967960 },
  { id: "PAK", ranking: 9, pais: "Pakistán", volumen: 23026100 },
  { id: "NZL", ranking: 10, pais: "Nueva Zelandia", volumen: 21051000 },
  { id: "TUR", ranking: 11, pais: "Turquía", volumen: 19912135 },
  {
    id: "GBR",
    ranking: 12,
    pais: "Reino Unido de Gran Bretaña e Irlanda del Norte",
    volumen: 15540640,
  },
  { id: "POL", ranking: 13, pais: "Polonia", volumen: 15208490 },
  {
    id: "NLD",
    ranking: 14,
    pais: "Países Bajos (Reino de los)",
    volumen: 14533740,
  },
  { id: "MEX", ranking: 15, pais: "México", volumen: 13497999 },
  { id: "ITA", ranking: 16, pais: "Italia", volumen: 13181870 },
  { id: "ARG", ranking: 17, pais: "Argentina", volumen: 11904142 },
  { ranking: '', pais: "Otros", volumen: 234005078 },
];

const agrupado = {};

tabla_paises_sub_prod.forEach((item) => {
  if (!agrupado[item.id]) {
    agrupado[item.id] = { ...item };
  } else {
    // Sumamos superficie
    agrupado[item.id].superficie += item.superficie;

    agrupado[item.id].volumen += item.volumen;
  }
});

// Convertimos el objeto agrupado a arreglo y ajustamos el ranking
const paises_datos_sub_pro = Object.values(agrupado).map((item, i) => ({
  ...item,
  ranking: i + 1,
}));

/* Datos para la primera tabla sobre la producción de carne de vaca a nivel mundial */
const tabla_paises_prod = [
  { id: "CHN", ranking: 1, pais: "China", volumen: 210590560.3 },
  { id: "CHN", ranking: 2, pais: "China, Continental", volumen: 210590547 },
  { id: "IND", ranking: 3, pais: "India", volumen: 70697673 },
  { id: "AUS", ranking: 4, pais: "Australia", volumen: 35647349 },
  { id: "TUR", ranking: 5, pais: "Türkiye", volumen: 26006878.71 },
  { id: "NZL", ranking: 6, pais: "Nueva Zelandia", volumen: 21979885 },
  { id: "NGA", ranking: 7, pais: "Nigeria", volumen: 19608513.42 },
  { id: "DZA", ranking: 8, pais: "Argelia", volumen: 18686454.13 },
  { id: "PAK", ranking: 9, pais: "Pakistán", volumen: 15777000 },
  { id: "SDN", ranking: 10, pais: "Sudán", volumen: 15353120.88 },
  { id: "GBR", ranking: 11, pais: "Reino Unido de Gran Bretaña e Irlanda del Norte", volumen: 14214500 },
  { id: "TCD", ranking: 12, pais: "Chad", volumen: 13574643.79 },
  { id: "IRN", ranking: 13, pais: "Irán (República Islámica del)", volumen: 12754573.82 },
  { id: "MAR", ranking: 14, pais: "Marruecos", volumen: 12436667 },
  { id: "ETH", ranking: 15, pais: "Etiopía", volumen: 12393123.2 },
  { id: "RUS", ranking: 16, pais: "Federación de Rusia", volumen: 10989686 },
  { id: "MNG", ranking: 17, pais: "Mongolia", volumen: 10799667 },
  { id: "UZB", ranking: 18, pais: "Uzbekistán", volumen: 9883978 },
  { id: "ESP", ranking: 19, pais: "España", volumen: 8475560 },
  { id: "SAU", ranking: 20, pais: "Arabia Saudita", volumen: 7687332.11 },
  { id: "BRA", ranking: 21, pais: "Brasil", volumen: 6982453.23 },
  { id: "KAZ", ranking: 22, pais: "Kazajstán", volumen: 6878538.13 },
  { id: "AFG", ranking: 23, pais: "Afganistán", volumen: 6562340.87 },
  { id: "TKM", ranking: 24, pais: "Turkmenistán", volumen: 6184708 },
  { id: "AZE", ranking: 25, pais: "Azerbaiyán", volumen: 6180951 },
  { id: "ROU", ranking: 26, pais: "Rumanía", volumen: 5725700 },
  { id: "IDN", ranking: 27, pais: "Indonesia", volumen: 5590799.82 },
  { id: "ZAF", ranking: 28, pais: "Sudáfrica", volumen: 5014493 },
  { id: "SYR", ranking: 29, pais: "República Árabe Siria", volumen: 4924011 },
  { id: "GRC", ranking: 30, pais: "Grecia", volumen: 4893180 },
  { id: "TJK", ranking: 31, pais: "Tayikistán", volumen: 4413046.72 },
  { id: "FRA", ranking: 32, pais: "Francia", volumen: 3803450 },
  { id: "YEM", ranking: 33, pais: "Yemen", volumen: 3772642.17 },
  { id: "TUN", ranking: 34, pais: "Túnez", volumen: 3757452.17 },
  { id: "KEN", ranking: 35, pais: "Kenya", volumen: 3641837 },
  { id: "MEX", ranking: 36, pais: "México", volumen: 3386713.27 },
  { id: "IRL", ranking: 37, pais: "Irlanda", volumen: 3238280 },
];

const agrupado2 = {};

tabla_paises_prod.forEach((item) => {
  if (!agrupado2[item.id]) {
    agrupado2[item.id] = { ...item };
  } else {
    // Sumamos superficie
    agrupado2[item.id].superficie += item.superficie;

    agrupado2[item.id].volumen += item.volumen;
  }
});

// Convertimos el objeto agrupado a arreglo y ajustamos el ranking
const paises_datos_pro = Object.values(agrupado2).map((item, i) => ({
  ...item,
  ranking: i + 1,
}));

/* Etiquetas para el periodo de tiempo usado en la grafica de balanza nacional */
const labels_balanza = [
  2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018, 2019, 2020, 2021, 2022,
];
/* Importaciones que del sector bovino */
const importacion_balanza = [];

/* Exportaciones del sector bovino */
const exportacion_balanza = [];

/* Etiquetas para el periodo de tiempo usado en la grafica de produccion historica */
const labels_historica_producto = [
  2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018, 2019, 2020, 2021, 2022,
  2023,
];

const labels_historica_sub_producto = [
  2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018, 2019, 2020, 2021, 2022,
  2023,
];

/* Valore para la grafica de produccion historica  producto principal carne*/
const produccion_historica_producto = [
  {
    nom: "Sierra de Florez Magón",
    fila: [
      884, 910, 936, 955, 970, 970, 971, 969, 998, 1004, 1016, 1011, 993, 1003,
    ],
  },
  {
    nom: "Costa",
    fila: [
      11155, 11580, 12232, 11969, 15217, 15205, 14742, 15473, 15702, 16102,
      16304, 16949, 16998, 17331,
    ],
  },
  {
    nom: "Istmo",
    fila: [
      10880, 11261, 12757, 12930, 13164, 13568, 14063, 14411, 14824, 15163,
      15736, 15937, 16398, 16782,
    ],
  },
  {
    nom: "Mixteca",
    fila: [
      5469, 5644, 4710, 4513, 4383, 5142, 5383, 5172, 5149, 5142, 5332, 5369,
      5500, 5557,
    ],
  },
  {
    nom: "Papaloapan",
    fila: [
      11768, 12227, 9983, 10059, 10052, 10203, 10790, 10993, 11932, 12908,
      12952, 12997, 13295, 13499,
    ],
  },
  {
    nom: "Sierra de Juárez",
    fila: [
      1401, 1438, 3760, 3880, 3472, 3554, 3640, 3706, 3781, 3948, 3951, 3980,
      4075, 4123,
    ],
  },
  {
    nom: "Sierra Sur",
    fila: [
      ,
      ,
      3082,
      3328,
      3193,
      3419,
      3573,
      3688,
      3746,
      3762,
      3927,
      3918,
      4029,
      4096,
    ],
  },
  {
    nom: "Valles Centrales",
    fila: [
      5491, 5677, 3730, 3681, 3660, 3605, 3678, 4090, 4279, 4350, 4519, 4611,
      4721, 4764,
    ],
  },
];

/* Valore para la grafica de produccion historica sub producto leche*/
const produccion_historica_sub_producto = [
  {
    nom: "Sierra de Florez Magón",
    fila: [
      2853, 2862, 2869, 2883, 2895, 2907, 2914, 2903, 2935, 2811, 2987, 3788,
      3622, 3627,
    ],
  },
  {
    nom: "Costa",
    fila: [
      19375, 19731, 20047, 19405, 23843, 23770, 23185, 23562, 25083, 25734,
      25737, 25535, 26633, 26986,
    ],
  },
  {
    nom: "Istmo",
    fila: [
      22312, 22412, 32760, 31947, 31274, 31742, 33838, 34767, 35429, 36037,
      37419, 36937, 37257, 37564,
    ],
  },
  {
    nom: "Mixteca",
    fila: [
      19109, 19148, 13099, 12581, 11840, 10520, 9938, 8928, 8016, 7435, 5946,
      6107, 6225, 6294,
    ],
  },
  {
    nom: "Papaloapan",
    fila: [
      24717, 24827, 19846, 20050, 20205, 18637, 18113, 16347, 17571, 17131,
      17096, 17676, 17574, 17763,
    ],
  },
  {
    nom: "Sierra de Juárez",
    fila: [
      3166, 3183, 9935, 9780, 10761, 11112, 11899, 12186, 12255, 12302, 12374,
      12681, 12910, 13100,
    ],
  },
  {
    nom: "Sierra Sur",
    fila: [
      ,
      ,
      14495,
      14689,
      14415,
      14541,
      14827,
      14841,
      14837,
      14772,
      14418,
      14638,
      14642,
      14767,
    ],
  },
  {
    nom: "Valles Centrales",
    fila: [
      55548, 55771, 34051, 33950, 33730, 32969, 32970, 31803, 31376, 31611,
      30495, 31447, 31290, 31559,
    ],
  },
];

/* DATO DE LA TABLA PRODUCCION  DE CARNE DE BOVINO EN OAXACA	 */

const tabla_produccion_producto = [
  {
    region: "Costa",
    volumen: 17331,
    peso: 227,
    precio: 77,
    valor: 1336119000,
    municipios: 45,
  },
  {
    region: "Istmo",
    volumen: 16782,
    peso: 199,
    precio: 74,
    valor: 1252731000,
    municipios: 41,
  },
  {
    region: "Papaloapan",
    volumen: 13499,
    peso: 184,
    precio: 74,
    valor: 1057978000,
    municipios: 20,
  },
  {
    region: "Mixteca",
    volumen: 5557,
    peso: 175,
    precio: 58,
    valor: 321471000,
    municipios: 155,
  },
  {
    region: "Valles Centrales",
    volumen: 4764,
    peso: 217,
    precio: 63,
    valor: 297873000,
    municipios: 119,
  },
  {
    region: "Sierra de Juárez",
    volumen: 4123,
    peso: 175,
    precio: 51,
    valor: 276452000,
    municipios: 65,
  },
  {
    region: "Sierra de Sur",
    volumen: 4096,
    peso: 209,
    precio: 65,
    valor: 259843000,
    municipios: 64,
  },
  {
    region: "Sierra de Flores Magón",
    volumen: 1003,
    peso: 176,
    precio: 85,
    valor: 84649000,
    municipios: 30,
  },
];

/* Datos grafica VOLUMEN DE PRODUCCION POR PUEBLO INDIGENA CARNE */
const pub_indigenas_producto = [
  [
    "NO PUEBLO INDIGENA",
    "ZAPOTECO",
    "MIXTECO",
    "CHINANTECO",
    "MIXE",
    "CHATINO",
    "MAZATECO",
    "ZOQUE",
    "CHONTAL DE OAXACA",
    "AMUZGO",
    "HUAVE",
    "NAHUA",
    "CHOCHOLTECO",
    "CUICATECO",
    "TRIQUI",
  ],
  [
    17837, 13426, 12301, 11557, 3431, 3279, 2895, 865, 418, 387, 360, 116, 77,
    68, 41,
  ],
];

/* Datos tabla PRODUCCION  DE LECHE DE VACA EN OAXACA	 */
const tabla_produccion_sub_producto = [
  {
    region: "Istmo",
    volumen: 37564000,
    precio: 8,
    valor: 309856000,
    municipios: 41,
  },
  {
    region: "Valles Centrales",
    volumen: 31559000,
    precio: 7,
    valor: 209183000,
    municipios: 120,
  },
  {
    region: "Costa",
    volumen: 26986000,
    precio: 8,
    valor: 219659000,
    municipios: 45,
  },
  {
    region: "Papaloapan",
    volumen: 17763000,
    precio: 7,
    valor: 126718000,
    municipios: 20,
  },
  {
    region: "Sierra Sur",
    volumen: 14767000,
    precio: 8,
    valor: 109145000,
    municipios: 64,
  },
  {
    region: "Sierra de Juárez ",
    volumen: 13100000,
    precio: 8,
    valor: 102664000,
    municipios: 66,
  },
  {
    region: "Mixteca",
    volumen: 6294000,
    precio: 8,
    valor: 51881000,
    municipios: 149,
  },
  {
    region: "Sierra de Flores Magón",
    volumen: 3627000,
    precio: 9,
    valor: 32049000,
    municipios: 32,
  },
];

/* Datos  grafica VOLUMEN DE PRODUCCION POR PUEBLO INDIGENA	LECHE*/
const pub_indigenas_sub_producto = [
  [
    "NO PUEBLO INDIGENA",
    "ZAPOTECO",
    "MIXTECO",
    "CHINANTECO",
    "MIXE",
    "CHATINO",
    "MAZATECO",
    "ZOQUE",
    "AMUZGO",
    "HUAVE",
    "CHONTAL DE OAXACA",
    "CUICATECO",
    "NAHUA",
    "TRIQUI",
    "CHOCHOLTECO",
  ],
  [
    43271, 42691, 20444, 16355, 12956, 8147, 3844, 1532, 660, 609, 538, 335,
    304, 36, 31,
  ],
];

/* Datos Grafica ESTACIONALIDAD DE LA PRODUCCION EN OAXACA CARNE */
const estacionalidad_producto = [
  108, 105, 104, 107, 112, 127, 143, 155, 159, 160, 156, 136,
];

const estacionalidad_sub_producto = [
  83.9, 90.2, 97.5, 95.9, 103.3, 108.9, 111.1, 111.2, 110.1, 113.2, 114.2,
  111.1,
];
