...
const getOptionChart6 = async () =>{
    try{
       const response = await fetch("http://127.0.0.1:8000/get_chart_2023_1/");
       return await response.json();
} catch (ex) {
       alert(ex);
}
...
//segunda grafica 2023

const getOptionChart22023 = async () =>{
     try{
        const response = await fetch("http://127.0.0.1:8000/getcharts22023/");
        return await response.json();
     } catch (ex) {
        alert(ex);
     }
...
const initChart22023 = async()=>{
     const myChart =echarts.init(document.getElementById("chart7"));
     myChart.setOption(await getOptionChart22023());
     myChart.resize();
 };

 window.addEventListener("load", async()=>{
     await initChart22023();
